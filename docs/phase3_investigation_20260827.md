...
## P3-C 薬価改定差分 — 完了（2026-08-27）

### 直したこと

`usePrintVisitData` と月次請求ワークベンチは、処方明細の薬価を
**現在のマスター薬価**から引いていた（`price: billingDrugDoc?.price || item.price || 0`）。
`PrescriptionItem` に薬価の項目は無いため、`item.price` は常に undefined で、
実質「常に現在のマスター薬価」だった。

このため薬価改定後にマスターを取り込むと、**過去の調剤分まで新薬価で再計算**されていた。
請求時点スナップショット差分に出ていた「算定設定やマスター更新により点数だけが変わっている
可能性があります」は、これを検出していた。

### 実装

| 層 | 内容 |
|---|---|
| モデル | `Drug.priceHistory: { price, effectiveFrom }[]`（drug schema v6 → v7） |
| 解決 | `src/lib/drug_price_history.ts` の `resolveDrugPrice(drug, 調剤日)`。改定日当日から新薬価 |
| 取込 | 医薬品マスターCSV取込で薬価が変わったときだけ版を積む（同値の再取込では増やさない） |
| 消費 | 印刷/UKE 出力（`usePrintVisitData`）と月次請求（`ClaimWorkbenchSection`）が調剤日で引く |

`resolveDrugPrice` は根拠も返す（`history` / `current` / `earliest_known` / `unknown`）。
履歴を持ち始める前の調剤は `earliest_known` になり、「分かっている中で最も古い薬価で代用した」
ことが呼び出し側に伝わる。調剤日が読み取れない場合は最新版を当てず `unknown` を返す。

### 残っていること

- ~~医薬品マスターCSVは改定日を持たない~~ → **2026-08-27 に対応**。
  項番30「変更年月日」を薬価の適用開始日として読むようにした（下記）。
- 既存の薬品は履歴が無いため `source: 'current'`（従来と同じ挙動）。
  履歴は次回のマスター取込から積まれる。


## 一部負担金額の入力経路 — 完了（2026-08-27）

P3-B で保留していた HO/KO の一部負担金額に、記録された値を出す経路を用意しました。
**算出はしません。** 窓口で徴収した額は高額療養費の現物給付・世帯合算・
処方元医療機関との関係で決まるためです（MF 窓口負担額情報と同じ扱い）。

| 記録先 | claimOptions の項目 |
|---|---|
| HO 第9項目 一部負担金 | `officialInsuranceCopaymentYen` |
| KO 第7・第9項目 | `officialPublicExpenseCopayments[]`（`patient.publicInsurances` と同じ並び） |

並びで対応させるため、公費の件数と一部負担金額の件数が合わない場合は
readiness error にして公式提出を止めます。

これらは `officialCopaymentRecords` など既存の公式専用項目と同じく、
RxDB スキーマには宣言していません（Ajv は未宣言の項目を通すため、
任意項目のためにスキーマ版を上げて全端末へ移行を強いる必要がないという判断です）。


## 薬価の適用開始日をマスターから取る — 完了（2026-08-27）

医薬品マスターCSVの **項番30 変更年月日** を、薬価の版の適用開始日として読むようにしました。
列が無い / 空 / 9999年（未設定）の行だけ取込日で代用します。
どちらを使ったかは監査ログに件数で残ります
（`薬価改定: N件（変更年月日 N件 / 取込日で代用 N件）`）。

**項番36「薬価基準収載年月日」ではありません。** こちらは初回収載日なので、
改定のたびに変わる値ではありません。

### 確認できたこと・できなかったこと

- 公式仕様PDF（`master_3_20260731.pdf`）を取得して、項番30 が「変更年月日」、
  項番36 が「薬価基準収載年月日」であることは確認しました（42列すべてリポジトリの定義と一致）。
- **ただし仕様PDFは列のレイアウト表だけで、各列の意味の説明がありません。**
  「変更年月日 = その内容が適用される日」という読み方は、レイアウト表からは確認できていません。
  遡って取り込む運用を始める前に、薬剤師・運用者に確認してください。

### ついでに直したこと

`DRUG_MASTER_SPECIFICATION_SOURCE.url` が `master_3_20260601.pdf` を指したままで、
**404 になっていました**（現在は `master_3_20260731.pdf`）。
仕様PDF取得（`/api/drug-master/official-spec-pdf`）と、それを使う列定義照合が動かない状態でした。
現在のURLへ差し替え、42列が一致することを確認しています。


## 薬価の版を請求画面で選べるようにした — 2026-08-27

調剤日時点の自動解決だけでは直せない場合（遡り取込で版がずれた等）に備えて、
処方薬ごとに薬価の版を選び直せるようにしました。点数が動く操作なので、
選び直したことが画面・監査ログ・請求前チェックの3か所に残ります。

| どこ | 何が出るか |
|---|---|
| 算定調整（薬剤情報提供文書の明細行） | 版のセレクタ。「調剤日時点（自動）」＋各版（自動選択の版には ※調剤日時点） |
| 同・明細行の下 | 選び直している間ずっと出る警告バー（適用中の薬価と、調剤日時点の薬価の両方を表示） |
| 監査ログ | `薬価の版変更: 薬品「…」に調剤日時点と異なる薬価を適用しました。…` |
| 請求前チェック | `drug_price_override_applied`（警告）。理由を摘要欄に記載してから提出する旨を出す |

保存先は `PrescriptionItem.drugPriceOverride`（prescription item schema v15 → v16）。
履歴が後から訂正されても適用した額が分かるよう、版の適用開始日と薬価の両方を持ちます。

### この作業で見つかった既存バグ 2件

実画面で操作して初めて分かったもので、どちらもテストは通っていました。

**1. 処方薬別の算定切替（調製・薬管・検査薬）が保存されていなかった**

`handleItemClaimToggle` が `currentItem.doc` を条件にしていましたが、画面の明細は
`usePrintVisitData` が `item.toJSON()` から組み立てており **RxDocument を持ちません**。
条件が常に false になり、チェックを操作しても何も起きない（エラーも出ない）状態でした。

実測: 調製のチェックを外す → 見た目も戻らない、監査ログも無い、リロードしても元のまま。

`applyItemClaimFlagWithAudit` の単体テストはモックの明細に `doc` を持たせていたため通っており、
`PrintPickingFlow.test.ts` の配線チェックも「呼んでいること」しか見ていませんでした。
DB から `prescription_items.findOne(itemId)` で引いて渡す形に直し、
doc が無い場合は理由の分かる例外にしました。

**2. 印刷画面で薬剤料が算定されていなかった**

`calculateDispensingFees` と `validateDispensingClaim` は明細の `drugPrice` を見ますが、
`usePrintVisitData` は `price` にしか入れていませんでした（`PrescriptionItem` に
`drugPrice` という項目は無く、ダッシュボード側の `dashboard_tasks.ts` だけが
`drugPrice: billingDrug?.price` と詰め直していました）。

実測: 請求前チェックが「アムロジピン錠5mg の薬価が未設定です（要修正）」「薬剤料が算定されていません」
をエラー4件として出し続けていた。修正後は エラー0件・警告0件（適合）になり、
調剤明細書に薬剤料 223点が載って合計 194点 → 417点 になりました。

**印刷帳票の寸法は11枚すべて基準と一致**しています（用紙サイズは固定のため）。
ただし点数が変わるので、既存の請求データを開くと表示点数が変わります。
