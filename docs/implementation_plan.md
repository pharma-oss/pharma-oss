...
### 4.2 Phase 2: UI構造刷新と真のテスト品質確立（1〜2ヶ月）
...
### 4.3 Phase 3: レセプト・マスタの堅牢化と本番運用体制（3〜6ヶ月）

| タスク ID | 改善項目 | 目的・内容 | 工数 (人日) | 期間 (暦日) |
|---|---|---|:---:|:---:|
| **P3-1** | **UKE/レセ電完全突合・返戻対応** | 支払基金・国保連仕様の全レコード（CZ/KI含む）と固定長/CSV仕様の網羅的バリデータ。返戻時のレセプト修正・差分スナップショット・再請求ワークベンチ | 15 人日 | 45 日 |
| **P3-2** | **医薬品マスタ差分検証・ロールバック** | 支払基金マスター更新時の自動差分解析（薬価改定・削除薬警告・代替マッピング）と、不整合発生時の 1 クリックロールバック保証 | 7 人日 | 30 日 |
| **P3-3** | **運用台帳と Nightly CI 統合** | 37 ファイルの CLI スクリプトを整理し、Nightly ドライランワークフローと `docs/ops_review_ledger.md`（運用台帳）へ集約 | 4 人日 | 20 日 |
