...
|---|---|---|
| 高額療養費 | 参照 **0 箇所** | **未実装** |
| 返戻ワークフロー | `claim_return_manager.ts` の R01-R99 体系 | **P3-A 完了（2026-08-27）**。§Phase 3 調査 参照 |
| 減免区分 | 参照 9 箇所 | 部分的 |
...
#### 仕様の所在 — 2026-08-27 の調査で確定

**「決定待ち」ではなかった。** 公式仕様は支払基金の記録条件仕様（令和8年6月版・調剤用）に確定しており、
本リポジトリはその仕様書を取得・解析する機構を既に持っている。詳細は
`docs/phase3_investigation_20260827.md`。

| 項目 | 確定している仕様 | 状態 |
|---|---|---|
| 高額療養費 | 別表7 レセプト特記事項コード（26区ア〜30区オ、31多ア〜35多オ、41区カ〜44多キ）／別表8 一部負担金区分 | **P3-B 完了（2026-08-27）**。区分の記録まで。額の算出はしない |
| 減免区分 | 別表10（1減額 / 2免除 / 3支払猶予）＋ 減額割合・減額金額・証明書番号 | **P3-B 完了（2026-08-27）** |
| 返戻ワークフロー | `claim_return_manager.ts` の R01-R99 体系（実装済みだった） | **P3-A 完了（2026-08-27）** |
| 薬価改定差分 | 調剤日時点の薬価 | **P3-C 完了（2026-08-27）**。`Drug.priceHistory` に適用開始日つきの版を持たせ、`resolveDrugPrice` で調剤日から引く |

実際の不足は「公式レイアウトの UKE ビルダが必須項目しか出力していない」ことに集約される
